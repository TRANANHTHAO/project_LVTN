...
<div class="container-fluid coupon form_ql">
    <div class="card_1">
        ...
        <div class="form-submit">
            <form action="{{ route('edit.coupon', $coupon->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6 col-12">
                        ...
                        <div class="promotion">
                            ...
                            <div class="form_group">
                                ...
                                <input name="ten" value="{{$coupon->ten}}" class="form_control" />
                            </div>
                            @if($errors->first('ten'))
                    ...
                    <div class="col-md-6 col-12">
                        ...
                        <div class="form_group">
                            <label>Mô tả</label>
                            <textarea class="form_control" name="mota" id="" rows="5">{{$coupon->mota}}</textarea>
                        </div>
                    ...
                    <div class="col-md-6 product_promotion {{+$coupon->dieukien === 1 ? '' : 'hide' }}">
                        <div class=" form_group">
                            ...
                            <h3 class="mt-2">Áp dụng với các sản phẩm</h2>
                                <div class="list-promo">
                                    <div class="list-cate-item">
                                        ...
                                        <input hidden value={{$value['cate']->id}} name="id_cates[]" />
                                        @foreach($value['products'] as $val)
                                        <div class="item_promo">
                                            <img class="img_promo" src="{{asset('uploads/product/' . $val->hinhanh)}}">
                                            <span class=" name_promo">{{$val->tensp}}</span>
                                            <input hidden value={{$val->id}} name="id_products[]" />
                                        </div>
                                        @endforeach
                                        @endforeach
                                        @else
...
window.onload = () => {
    ...
    const getListPromotion = {
        ...
        getData: async function() {
            const response = await fetch('http://localhost/website_quanlycoffee/public/getListData');
        ...
        renderCate: function() {
            ...
            render += this.category.map((item, index) => {
                ...
                return (`
                    <tr>
                    <td scope=" col"><input data-id="${item.id}" name="checkedAll[]" ${checked}  type="checkbox" /></td>
                    <td>${index}</td>
